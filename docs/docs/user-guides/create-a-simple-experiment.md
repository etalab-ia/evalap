...
:::info
Query the complete metrics list from the [v1/metrics](/redoc#tag/metrics) API route.
:::
...
The model schema support passing sampling params, such as the temperature like `"model": {..., "sampling_params": {"temperature": 0.2}}`, or extra params since supported by the Openai API used. Check the [experiment creation endpoint](/redoc#tag/experiments/operation/create_experiment_v1_experiment_post) to full list of supported parameters.
:::
...
See the API documention of the ModelRaw schema to see all the parameters accepted for a custom model: [create_experiment_v1_experiment_post](/redoc#tag/experiments/operation/create_experiment_v1_experiment_post)
:::
