...
Options:
    ...
    --run-name=<name>             name of the model generation to load (result generated by eg1_run_answers).
...
def run_expe(args):
    ...
    if expid is None:
        ...
        # PATCH the expset
        response = requests.patch(f"{eg1_url}/experiment_set/{expid}", json=expset, headers=headers)
        log_and_raise_for_status(response, "EG1 error")
        resp = response.json()
