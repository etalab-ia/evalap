...
class TestEcoCarbon(TestApi):
    @patch("evalap.core.utils_eco.load_models_info", return_value=MOCK_MODELS_INFO)
    @patch("evalap.core.utils_eco.electricity_mixes.find_electricity_mix", return_value=MOCK_ELECTRICITY_MIX)
    @patch("evalap.core.utils_eco.compute_llm_impacts", return_value=MOCK_IMPACT_RESULT)
    def test_impact_carbon_albert(self, mock_compute, mock_elec, mock_models):
        result = evalap.core.utils_eco.impact_carbon("albert", "https://modelhost/albert", 50, 0.5)
    ...
    def test_impact_carbon_other_model(self, mock_compute, mock_elec, mock_models):
        result = evalap.core.utils_eco.impact_carbon("other", "https://modelhost/other", 100, 1.0)
        assert result["gwp"] == 42
        assert result["estimated"] is False
        mock_elec.assert_called_with(zone="WOR")

    @patch("evalap.core.utils_eco.load_models_info", return_value={})
    @patch("evalap.core.utils_eco.electricity_mixes.find_electricity_mix", return_value=MOCK_ELECTRICITY_MIX)
    @patch("evalap.core.utils_eco.compute_llm_impacts", return_value=MOCK_IMPACT_RESULT)
    def test_impact_carbon_estimation(self, mock_compute, mock_elec, mock_models):
        result = evalap.core.utils_eco.impact_carbon("unknown-model-large", "https://modelhost/unknown", 10, 0.1)
    ...
    def test_impact_carbon_token_count_invalid(self, mock_compute, mock_elec, mock_models):
        with pytest.raises(ValueError):
            evalap.core.utils_eco.impact_carbon("albert", "https://modelhost/albert", -1, 0.1)
    ...
    def test_impact_carbon_latency_invalid(self, mock_compute, mock_elec, mock_models):
        with pytest.raises(ValueError):
            evalap.core.utils_eco.impact_carbon("albert", "https://modelhost/albert", 10, -0.1)

    @patch("evalap.core.utils_eco.load_models_info", return_value=MOCK_MODELS_INFO)
    @patch("evalap.core.utils_eco.electricity_mixes.find_electricity_mix", return_value=None)
    @patch("evalap.core.utils_eco.compute_llm_impacts", return_value=MOCK_IMPACT_RESULT)
    ...
    def test_impact_carbon_model_url_not_string(self, mock_compute, mock_elec, mock_models):
        with pytest.raises(ValueError):
            evalap.core.utils_eco.impact_carbon("albert", 12345, 10, 0.1)
