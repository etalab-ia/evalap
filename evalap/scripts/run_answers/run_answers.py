...
"""
Generate output/answers with evalap dataset with the given model.

Usage:
    run_answers.py --run-name=<name> --base-url=<url> --model=<model> --dataset=<dataset> [--auth-token=<token>] [--system-prompt=<prompt>] [--sampling-params=<params>] [--extra-params=<params>] [--max-workers=<n>] [--evalap-token=<token>] [--repeat=<n>]
...
Examples:
    evalap_generate_answers --base-url http://localhost:9191/v1 --model google/gemma-3-27b-it --dataset MFS_questions_v01 --repeat 4 --run-name gemma-3-27b_mfs
"""
...
def run_model(args):
    ...
    response = requests.get(
        f"{evalap_url}/dataset?name={dataset_name}&with_df=true",
        headers={"Authorization": f"Bearer {evalap_token}"},
    )
